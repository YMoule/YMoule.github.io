...
First, I analyzed the data available in the Fintech dataset using BigQuery. I then joined a CSV table with state and region information to the Fintech dataset, using a inner join on state. It is important to know the region of each loan, since many loans in one region carries of risk of many loans defaulting at once.
Finally, I concluded the prelimary report by creating a table identifying the number of loans issued by day and year. 

<img src="images/Fintech_Table.png?raw=true"/>

### 2. The Dashboard

An interactive dashboard was created using Looker and LookML to allow the treasury department to access key performance metrics. Most importantly, the dashboard includes a single-value visualization of the total amount of outstanding loans. It is highlighted in red in the screenshot below because the amount of outstanding loans is above the thresheld set by the Treasury Department. 
